...
class _QuizScreenState extends State<QuizScreen> {
  ...
  Future<void> _loadQuestions() async {
    try {
      ...
      for (var questionData in questionsList) {
        ...
        // Structure-based type detection (most reliable)
        bool isCardMatchByStructure = false;
        bool isMatchFollowingByStructure = false;
        bool isSequenceBuilderByStructure = false;
        bool isSimulationByStructure = false;
        bool isFlipCardByStructure = false;
        
        if (optionsRaw is Map) {
          // Card match: {cards: [...], buckets: [...]}
          if (optionsRaw.containsKey('cards') && optionsRaw.containsKey('buckets')) {
            isCardMatchByStructure = true;
          }
          // Budget simulation: {total_budget: int, departments: [...]}
          if (optionsRaw.containsKey('total_budget') && optionsRaw.containsKey('departments')) {
            isSimulationByStructure = true;
          }
        }
        if (optionsRaw is List && optionsRaw.isNotEmpty && optionsRaw[0] is Map) {
          ...
          if (first.containsKey('left') && first.containsKey('right')) {
            isMatchFollowingByStructure = true;
          }
          // Sequence builder: [{id, text, correct_position}, ...]
          if (first.containsKey('text') && first.containsKey('correct_position')) {
            isSequenceBuilderByStructure = true;
          }
          // Card flip game: [{id, question, answer}, ...]
          if (first.containsKey('question') && first.containsKey('answer')) {
            isFlipCardByStructure = true;
          }
        }
        
        if (isSimulationByStructure) {
          inferredType = 'simulation';
          debugPrint('   ✅ Budget Simulation detected by OPTIONS STRUCTURE (total_budget/departments)');
        } else if (isMatchFollowingByStructure) {
          inferredType = 'match_following';
          debugPrint('   ✅ Match Following detected by OPTIONS STRUCTURE (left/right keys)');
        } else if (isSequenceBuilderByStructure) {
          inferredType = 'sequence_builder';
          debugPrint('   ✅ Sequence Builder detected by OPTIONS STRUCTURE (text/correct_position)');
        } else if (isFlipCardByStructure) {
          inferredType = 'card_match';
          debugPrint('   ✅ Card Flip detected by OPTIONS STRUCTURE (question/answer)');
        } else if (isCardMatchByStructure) {
      ...
      // Skip options processing for card_match, sequence_builder, simulation, and match_following questions - they use different structures
      if (inferredType != 'card_match' && inferredType != 'sequence_builder' && inferredType != 'simulation' && inferredType != 'match_following' && questionData['options'] != null) {
        final optionsRaw = questionData['options'];
      ...
      } else if (inferredType == 'match_following') {
        debugPrint('  Match Following question - preserving List structure for match_pairs');
      }
        ...
        questions.add({
          ...
          'options': (inferredType == 'card_match' || inferredType == 'sequence_builder' || inferredType == 'simulation' || inferredType == 'match_following') ? questionData['options'] : options,
          'options_data': optionsData,
          'correct_answer': questionData['correct_answer'],
          'question_type': inferredType,
          'card_pairs': questionData['options'], // For Card Match questions
          'match_pairs': inferredType == 'match_following' ? questionData['options'] : null, // For Match Following questions
        });
      }
                            ...
                            ElevatedButton(
                              onPressed: !isAnswered
                                  ...
                                  : () async {
                                      ...
                                      // Handle match_following questions
                                      if (questionType == 'match_following') {
                                        final pairs = List<Map<String, dynamic>>.from(question['match_pairs'] ?? []);
                                        final userMatches = _matchAnswers[_currentQuestionIndex] ?? {};
                                        
                                        // Check correctness: each left item must be matched to its correct right item
                                        int correctCount = 0;
                                        for (var pair in pairs) {
                                          final leftItem = pair['left'] as String;
                                          final correctRight = pair['right'] as String;
                                          final userRight = userMatches[leftItem];
                                          // userRight has format "rightValue|index", extract the value
                                          final userRightValue = userRight?.split('|')[0];
                                          if (userRightValue == correctRight) correctCount++;
                                        }
                                        
                                        final bool allCorrect = correctCount == pairs.length;
                                        final int points = allCorrect ? _calculatePoints(_currentQuestionIndex) : 0;
                                        ...
                                        setState(() {
                                          _answeredCorrectly[_currentQuestionIndex] = allCorrect;
                                        });
  ...
  Widget _buildResultsScreen() {
    ...
    // Calculate total score: combine time-based points (MCQ/match) + game scores (card_match/sequence/simulation)
    int totalScore = 0;
